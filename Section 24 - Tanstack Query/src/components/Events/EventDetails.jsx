...
export default function EventDetails() {
 ...
 const {mutate, isPending: isPendingDeletion, isError: isErrorDeleting, error: deleteError} = useMutation({
  ...
  if(isError){
    content = <div id='event-details-content' className='center'>
    <ErrorBlock title='Failed to load event' message={error.info?.message || 'Failed to fetch events data, please try again'}/>
  </div>
  }
...
<>
  ...
  return (
    ...
    {isDeleting && (<Modal onClose={handleStopDelete}>
      <h2>Are you sure?</h2>
      <p>Do you really want to delete this? This action can't be undone</p>
